...
function init(config, dependencies) {
  ...
  async function buildAndSendTx(
    ...
    const functionCallData = buildFunctionCallData(
      abi,
      functionName,
      args,
      web3
    ...
    try {
      ...
      logger.log(
        "warn",
        "Unable to estimate gas for function call [" +
          functionName +
          "]: " +
  ...
  function buildFunctionCallData(abi, functionName, args, web3) {
    try {
      ...
      if (Array.isArray(args)) {
        ...
        const e = Error(
          "Unsupported format for function call arguments, please format correctly"
        );
        logger.log("error", e);
