  ...
  for (let c in contracts) {
    console.log(
      "Deploying contract [" +
        c +
        "] via provider [" +
        context.web3.currentProvider.host +
        "]"
    );
    ...
    try {
      let result = await sendTransaction(context, {
        // Will deploy the contract. The promise will resolve with the new contract instance, instead of the receipt!
  ...
  try {
    ...
    if (!!error.reason) {
      ...
      if (revertReasonHex) {
        ...
        revertReasonHex = revertReasonHex.startsWith("0x")
          ? revertReasonHex
          : "0x" + revertReasonHex;
...
async function invokeMethod(context, contract, senderAddress, method, args) {
  ...
  if (args.length > 0)
    await contract.methods[method](...args)
      ...
      .on("error", (err) => {
        console.log(
          "Invoking method [" + method + "] error [" + err.message() + "]"
        );
      })
      .then((receipt) => {
        console.log(
          "Invoking method [" + method + "] receipt [" + receipt.status + "]"
        );
      });
  else
...
async function callMethod(context, contract, senderAddress, method, args) {
  ...
  else return await contract.methods[method]().call(params);
}
...
async function setMessagingContractAddress(
  context,
  fcContract,
  senderAddress,
  contractAddress
...
async function setFunctionCallContractAddress(
  context,
  xvpContract,
  senderAddress,
  contractAddress
  ...
  await invoke(
    ...
    xvpContract,
    senderAddress,
    "setFunctionCallContractAddress",
    args
  );
}

async function setTokenContractAddress(
  context,
  xvpContract,
  senderAddress,
  contractAddress
...
async function setRemoteAccountIdToLocalAccountId(
  context,
  xvpContract,
  senderAddress,
  remoteAccountId,
...
async function getRemoteAccountIdToLocalAccountId(
  context,
  tokenContract,
  senderAddress,
  remoteAccountId
...
async function onboardProvingScheme(
  context,
  msgContract,
  senderAddress,
  networkId,
  ...
  await invoke(
    ...
    msgContract,
    senderAddress,
    "onboardProvingScheme",
    args
  );
}

async function onboardEventDecodingScheme(
  context,
  fcContract,
  senderAddress,
  networkId,
  ...
  await invoke(
    ...
    fcContract,
    senderAddress,
    "onboardEventDecodingScheme",
    args
  );
}

async function setParameterHandlers(
  context,
  msgContract,
  senderAddress,
  networkId,
...
async function getParameterHandler(
  context,
  msgContract,
  senderAddress,
  networkId,
...
async function getFunctionCommand(
  context,
  msgContract,
  senderAddress,
  networkId,
...
async function getFunctionPrototype(
  context,
  msgContract,
  senderAddress,
  networkId,
...
async function setValidatorList(
  context,
  msgContract,
  senderAddress,
  networkId,
...
async function addHoldNotary(
  context,
  tokenContract,
  senderAddress,
  notaryId,
  holdNotaryAdminAddress
) {
  let args = {
    notaryId: notaryId,
...
async function setLocalNetworkId(
  context,
  imContract,
  senderAddress,
  networkId
...
async function addRemoteDestinationNetwork(
  context,
  imContract,
  senderAddress,
  networkId,
...
async function enableRemoteDestinationNetwork(
  context,
  imContract,
  senderAddress,
  networkId
...
async function listRemoteDestinationNetworks(
  context,
  imContract,
  senderAddress
) {
...
async function setInteropManager(
  context,
  vsmContract,
  senderAddress,
  contractAddress
...
async function setupConfig(context, config, contracts) {
  ...
  assert.strictEqual(
    await getOwner(context, contracts.crosschainFunctionCall, from),
    from.toLowerCase(),
    msg
  );
  assert.strictEqual(
    await getOwner(context, contracts.crosschainXvP, from),
    from.toLowerCase(),
    msg
  );
  assert.strictEqual(
    await getOwner(context, contracts.assetTokenContract, from),
    from.toLowerCase(),
    msg
  );

  await getValidators(context, contracts.validatorSetManager, from);
  await setLocalNetworkId(
    context,
    contracts.interopManager,
    from,
    config.localNetworkId
  ); // Set the local network id
  await setLocalNetworkId(
    context,
    contracts.crosschainFunctionCall,
    from,
    config.localNetworkId
  ); // Set the local system id
  await setAppendAuthParams(
    context,
    contracts.crosschainFunctionCall,
    from,
    true
  ); // Enable contract authentication
  await setMessagingContractAddress(
    context,
    contracts.crosschainFunctionCall,
    from,
    contracts.crosschainMessaging.address
  );
  await setNotaryId(
    context,
    contracts.crosschainXvP,
    from,
    config.holdNotaryId
  );
  await setFunctionCallContractAddress(
    context,
    contracts.crosschainXvP,
    from,
    contracts.crosschainFunctionCall.address
  );
  await setTokenContractAddress(
    context,
    contracts.crosschainXvP,
    from,
    contracts.assetTokenContract.address
  );
  await addHoldNotary(
    context,
    contracts.assetTokenContract,
    from,
    config.holdNotaryId,
    contracts.crosschainXvP.address
  );
  await createTokens(
    context,
    contracts.assetTokenContract,
...
async function setupRemote(context, config, contracts) {
  let from = config.deployerAddress;
  await setInteropManager(
    context,
    contracts.validatorSetManager,
    from,
    contracts.interopManager.address
  );
  for (let remoteNetwork of config.remoteNetworks) {
    if (remoteNetwork.cordaNetworkId !== undefined) {
      await addRemoteDestinationNetwork(
        context,
        contracts.interopManager,
        from,
        remoteNetwork.cordaNetworkId,
        remoteNetwork.connectorContract
      ); // This is the messaging contract currently
      await enableRemoteDestinationNetwork(
        context,
        contracts.interopManager,
        from,
        remoteNetwork.cordaNetworkId
      ); // Misuse of enable/disable of connector contract to enable/disable auth parameters
      await listRemoteDestinationNetworks(
        context,
        contracts.interopManager,
        from
      );
      ...
      await onboardEventDecodingScheme(
        context,
        contracts.crosschainFunctionCall,
        from,
        remoteNetwork.cordaNetworkId,
        1
      ); // Corda decoding scheme
      await onboardProvingScheme(
        context,
        contracts.crosschainMessaging,
        from,
        remoteNetwork.cordaNetworkId,
        1
      ); // Corda transaction-based proving scheme
      await addParticipant(
        context,
        contracts.crosschainMessaging,
        from,
        remoteNetwork.cordaNetworkId,
        remoteNetwork.cordaPartyAKey
      ); // Corda issuing party
      await addParticipant(
        context,
        contracts.crosschainMessaging,
        from,
        remoteNetwork.cordaNetworkId,
        remoteNetwork.cordaPartyBKey
      ); // Corda receiving party
      await addNotary(
        context,
        contracts.crosschainMessaging,
        from,
        remoteNetwork.cordaNetworkId,
        remoteNetwork.cordaNotaryKey
      ); // Corda notary
      for (let h of remoteNetwork.cordaParameterHandlers) {
        await setParameterHandlers(
          context,
          contracts.crosschainMessaging,
          from,
          remoteNetwork.cordaNetworkId,
      ...
      await setRemoteAccountIdToLocalAccountId(
        context,
        contracts.crosschainXvP,
        from,
        remoteNetwork.cordaPartyARemoteId,
        remoteNetwork.cordaPartyALocalId
      ); // Register Party A id mapping
      assert.strictEqual(
        await getRemoteAccountIdToLocalAccountId(
          context,
          contracts.crosschainXvP,
          from,
          remoteNetwork.cordaPartyARemoteId
      ...
      await setRemoteAccountIdToLocalAccountId(
        ...
        contracts.crosschainXvP,
        from,
        remoteNetwork.cordaPartyBRemoteId,
        remoteNetwork.cordaPartyBLocalId
      ); // Register Party B id mapping
      assert.strictEqual(
        await getRemoteAccountIdToLocalAccountId(
          context,
          contracts.crosschainXvP,
    ...
    if (remoteNetwork.ethNetworkId !== undefined) {
      ...
      for (let i = 0; i < remoteNetwork.authenticatedContracts.length; i++) {
        await addAuthParams(
          ...
          from,
          remoteNetwork.ethNetworkId,
          remoteNetwork.authenticatedContracts[i]
        );
      }
      await onboardEventDecodingScheme(
        context,
      ...
      await onboardProvingScheme(
        ...
        remoteNetwork.ethNetworkId,
        2
      ); // Block header proving scheme
      await setValidatorList(
        context,
        contracts.crosschainMessaging,
        from,
        remoteNetwork.ethNetworkId,
        remoteNetwork.ethValidatorAddresses
      ); // Register validator list
      await setRemoteAccountIdToLocalAccountId(
        context,
        contracts.crosschainXvP,
        from,
        remoteNetwork.ethPartyARemoteId,
        remoteNetwork.ethPartyALocalId
      ); // Register Party A id mapping
      assert.strictEqual(
        await getRemoteAccountIdToLocalAccountId(
          context,
      ...
      await setRemoteAccountIdToLocalAccountId(
        ...
        contracts.crosschainXvP,
        from,
        remoteNetwork.ethPartyBRemoteId,
        remoteNetwork.ethPartyBLocalId
      ); // Register Party B id mapping
      assert.strictEqual(
        await getRemoteAccountIdToLocalAccountId(
          context,
...
async function updateConfig(pathToConfig, networkId, deployed) {
  ...
  console.log(
    "Updated system [" + systemName + "] config in [" + pathToConfig + "]"
  );
}
